...
#   الإصدار: 21 (حذف تلقائي للتسليمات القديمة عند كوفينغ جديد)
# =====================================================================
...
BOT_TOKEN = "8979319753:AAEnlYOahd7TkvET9a9BE4A4khkWtjmNmaE"  # ⚠️ أبطِله!
OWNER_ID = 8633059017
DEVELOPER_USERNAME = "YourDevUsername"
DB_PATH = "bot_database.db"
USERS_PER_PAGE = 5
DEFAULT_REQUIRED_REACTIONS = 100
API_ID = 39053073
API_HASH = "9a2522d03990c82545df5d5df257eb6f"
...
class Database:
        ...
        async with aiosqlite.connect(self.path) as db:
            await db.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS user_messages (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
                    username TEXT, name TEXT, message TEXT, created_at TEXT,
            ...
            await self._ensure_columns(db, "user_messages", {
                "username": "TEXT", "name": "TEXT",
                "is_replied": "INTEGER DEFAULT 0",
            })
            await db.executescript("""
    ...
    # ---------- User Messages ----------
    async def save_user_message(self, uid, username, name, message):
        async with aiosqlite.connect(self.path) as db:
            await db.execute("""INSERT INTO user_messages (user_id, username, name, message, created_at)
                VALUES (?, ?, ?, ?, ?)""", (uid, username, name, message[:1000], self._now()))
            await db.commit()
    ...
    # ✅ NEW: حذف التسليمات القديمة
    async def delete_old_claims(self, keep_config_id: int) -> int:
        """يحذف كل التسليمات الخاصة بكوفينغات قديمة (ما عدا الحالي)."""
        async with aiosqlite.connect(self.path) as db:
...
async def safe_edit(call, text, reply_markup=None, disable_web_page_preview=False):
    """يعدّل رسالة، وإن فشل (لأنها ملف/صورة) يحذفها ويرسل رسالة جديدة."""
    try:
...
def reply_to_user_kb(uid):
    kb = InlineKeyboardBuilder()
    kb.button(text="↩️ الرد على المستخدم", callback_data=f"reply:to:{uid}")
    return kb.as_markup()
...
async def notify_admins_new_message(bot, user, text):
    msg = (f"📩 <b>رسالة جديدة من مستخدم</b>\n\n"
           f"👤 @{user.username or '—'}\n"
           f"🆔 <code>{user.id}</code>\n"
           f"📛 {user.full_name}\n\n"
           f"💬 <b>الرسالة:</b>\n{text}")
    kb = reply_to_user_kb(user.id)
    targets = [OWNER_ID] + await db.admin_ids()
    for tid in set(targets):
        if tid == user.id:
            continue
        try:
            await bot.send_message(tid, msg, reply_markup=kb)
        except Exception as e:
            logger.debug(f"notify {tid}: {e}")
...
async def user_general_message(message: Message, state: FSMContext, bot: Bot):
    user = message.from_user
    if is_owner(user.id) or await db.is_admin(user.id):
        return
    ...
    if await db.is_banned(user.id):
        return
    await db.save_user_message(user.id, user.username, user.full_name, message.text)
    await notify_admins_new_message(bot, user, message.text)
    await message.answer("📩 تم إرسال رسالتك للإدارة، سيتم الرد عليك قريباً.")
...
# ----- Reply to user -----
@admin_router.callback_query(F.data.startswith("reply:to:"))
async def cb_reply_to(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"↩️ <b>أرسل الآن الرد على المستخدم</b>\n"
        f"🆔 <code>{target_id}</code>\n\n"
        f"سيتم إرسال نص رسالتك مباشرة للمستخدم.",
        reply_markup=cancel_kb("admin:panel"))
    await call.answer()


@admin_router.message(ReplyFSM.waiting)
async def process_reply(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await message.answer("❌ المستخدم حظر البوت، لا يمكن إرسال الرد.")
    except Exception as e:
...
async def cfg_watch_link(message: Message, state: FSMContext, bot: Bot):
    ...
    # ✅ حذف التسليمات القديمة تلقائياً (توفير موارد)
...
async def cmd_check(message: Message, bot: Bot):
    ...
    if count is None:
        await message.answer("❌ فشل القراءة."); return
    await db.set_reaction_count(cfg["id"], count)
